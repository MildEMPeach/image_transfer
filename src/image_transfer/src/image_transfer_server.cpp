...
public:
                         ...
                         image_transfer_srv::Response::SharedPtr response)
    {
        ...
        {   
            ...
            const int origin_width = request->origin_width;
            const int origin_height = request->origin_height;
            const bool test = request->test;
            const auto &image_data = request->data;

            // Deal with data
            if (compression_type == "none")
            {
                // Read raw data directly
                decoded_cv_image = cv::Mat(origin_height, origin_width, CV_8UC3, (void *)image_data.data()).clone();
            }
            else if (compression_type == "jpeg")
            {
                const std::vector<uint8_t> buffer(
